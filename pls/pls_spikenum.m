...
for sub_id = 1:length(cur_pat_ids)
    ...
    for ises = 1:2
        ...
        rel_ids = find(pe_table.Participant==cid &...
             pe_table.brainArea<3 &pe_table.Session==ises);
        ...
        if length(rel_ids)>=1
            ...
            for ineu = 1:length(all_sub_neu_ids{sub_id, ises})
              ...
              % extract spike sum
            spike_sum_dat{ises} = sum(pe_table.psth10(rel_ids,1:100),2);
            % re-arrange data
            spike_sum_mat{ises} = reshape(spike_sum_dat{ises} , num_tr_nums(ises),...
                length(sub_neus{ises}));
            
            % extract PE (only needed to make sure same trials / neurons 
            % are ecluded as in PE models)
            ...
            spike_sum_mat{ises}(:,(mean(re_mat_10_2{ises}))<.0001) = [];
            ...
            if size(re_mat_10_2{ises},2)>1
                ...
                % run PLS models
                % neural matrix consists of spike numbers across trials and
                % neurons
                datamat_lst = {cell2mat(spike_sum_mat(ises))};
...
% save stuff
save('SpikeVar_early_vs_late_Spikenum_PLS_results.mat', 'behav_weights',...
