...
% calculate responses for each patch
    c1_mean = cellfun(@(x) mean(x,3), c1{1}, 'uni', 0); 
    
    patch_estim.c1_median = cell2mat(cellfun(@(x) median(x(:)), c1_mean, 'uni', 0)); % take median
    patch_estim.c2_median = cell2mat(cellfun(@median, c2, 'uni', 0)); % take median
    ...
    patch_estim.c2_sd = cell2mat(cellfun(@std, c2, 'uni', 0)); % take std    
end
